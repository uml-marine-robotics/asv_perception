...
# WaSR NO-IMU model
# - ResNet101 backbone with atrous convolutions
# - Proposed decoder with ARM and FFM modules (Initial idea of ARM and FFM module were presented by Yu, Changqian, et al. ECCV 2018)
# - ASPP module at the end of the decoder to capture wider spatial context

# Weights of ResNet101 and ASPP are pretrained on ImageNet

from kaffe.tensorflow import Network
import tensorflow as tf

class wasr_NOIMU2(Network):
    def setup(self, is_training, num_classes):
        ...
        (self.feed('res5c_relu')
             #.attention_refinment_module(name='arm'))
             .attention_refinment_module_new(name='arm'))

        (self.feed('res3b3_relu')
             .attention_refinment_module_new(name='2_arm', last_arm=True)
             .conv(1, 1, 2048, 1, 1, biased=False, relu=False, name='arm_2_upfeatures'))
             #.batch_normalization(is_training=is_training, activation_fn=None, name='bn_arm_2_upfeatures'))
        ...
        (self.feed('res2c',
                   'combine_arm1_arm2')
             #.feature_fusion_module(name='ffm'))
