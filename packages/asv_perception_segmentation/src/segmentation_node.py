...
class segmentation_node(object):
    ...
    def __init__(self):
        ...
        # publishers
        self.pubs_obstacles = [ rospy.Publisher( '~%d/obstacles' % i, Image, queue_size=1 ) for i in range(self.n_inputs) ]
        self.pubs_water = [ rospy.Publisher( '~%d/water' % i, Image, queue_size=1 ) for i in range(self.n_inputs) ]
        self.pubs_sky = [ rospy.Publisher( '~%d/sky' % i, Image, queue_size=1 ) for i in range(self.n_inputs) ]
    ...
    def cb_sub( self, msg, idx ):
        ...
        img = utils.resize( img, wasr_img_shape )[0]
        ...
        if pub_obs.get_num_connections() > 0:
            out = self.remap_pixel_class( img, 0, self.pixel_remap_value, orig_shape )
            out.header = msg.header
            pub_obs.publish( out )

        if pub_water.get_num_connections() > 0:
            out = self.remap_pixel_class( img, 1, self.pixel_remap_value, orig_shape )
            out.header = msg.header
            pub_water.publish( out )

        if pub_sky.get_num_connections() > 0:
            out = self.remap_pixel_class( img, 2, self.pixel_remap_value, orig_shape )
            out.header = msg.header
            pub_sky.publish( out )
